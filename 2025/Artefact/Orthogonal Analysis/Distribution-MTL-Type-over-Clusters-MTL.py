...
# Convert MTLtype_count_dict to list of lists (same order as MTLtype)
...
for i, mt in enumerate(MTLtype):
    for j, cluster in enumerate(MTLCluster):
        ...
        a1.annotate(str(text), xy=(cluster, mt), ha='center', va='center', color='black', size=14)
...
for i, tool in enumerate(TOOLCluster):
    for j, cluster in enumerate(MTLCluster):
        ...
        text = str(count) if count != 0 else " "
        a2.annotate(str(text), xy=(cluster, tool), ha='center', va='center', color='black', size=14)
